...
function rememberSessionHint() {
  ...
  if (name && role) sessionStorage.setItem('fitosanidad-session-hint', JSON.stringify({ name, role }));
}
...
async function loadData(force = false) {
  ...
  try {
    ...
    const requests = [];
    if (force || !state.records.length) requests.push(getCentralSnapshot(state.user, 5000).then((data) => { state.records = data.records || []; }));
    if (!state.catalog.length) requests.push(fetch('./data/catalogo-lotes.json').then((r) => { if (!r.ok) throw new Error('No se pudo cargar el catálogo de lotes.'); return r.json(); }).then((data) => { state.catalog = data; }));
    if (!state.geojson) requests.push(fetch('./data/lotes-mapa.geojson').then((r) => { if (!r.ok) throw new Error('No se pudo cargar el mapa de lotes.'); return r.json(); }).then((data) => { state.geojson = data; }));
...
function featureMatches(feature) {
  ...
  return state.catalog.some((row) => row.lote === p.LOTE);
}
...
function filterOptions() {
  ...
  state.records.forEach((r) => { const id = r.evaluadorId || r.evaluador; if (id) evaluators.set(id, r.evaluador || r.evaluadorId); });
...
function aggregate(rows) {
  return {
    ...
    captures: rows.reduce((s,r) => s + Number(r.capturas || 0), 0),
    traps: rows.reduce((s,r) => s + Number(r.trampasRevisadas || 0), 0),
...
function typeSummary(rows, type) {
  ...
  const captures = typed.reduce((s,r) => s + Number(r.capturas || 0), 0);
  const traps = typed.reduce((s,r) => s + Number(r.trampasRevisadas || 0), 0);
  const avg = typed.length ? typed.reduce((s,r) => s + Number(r.indicador || 0), 0) / typed.length : 0;
  return { count: typed.length, captures, traps, avg };
}

function kpisMarkup(rows) {
  const a = aggregate(rows);
  const type = state.filters.type;
  const avg = type === 'all' || !rows.length ? null : rows.reduce((s,r) => s + Number(r.indicador || 0), 0) / rows.length;
...
function bars(title, subtitle, entries, unit = '') {
  const top = entries.slice(0, 8);
  const max = Math.max(1, ...top.map((x) => x[1]));
  return `<article class="sup-card sup-chart"><div class="sup-chart-head"><h3>${esc(title)}</h3><span>${esc(subtitle)}</span></div>${top.length ? `<div class="sup-bars">${top.map(([label,value]) => `<div class="sup-bar-row"><div class="sup-bar-label" title="${esc(label)}">${esc(label)}</div><div class="sup-bar-track"><i style="width:${Math.max(3,(value/max)*100).toFixed(1)}%"></i></div><b>${fmt(value)}${unit}</b></div>`).join('')}</div>` : '<div class="sup-empty">Sin datos con estos filtros.</div>'}</article>`;
}
...
function weeklyChart(rows) {
  ...
  rows.forEach((r) => {
    ...
    if (!key || key === '-S00') return;
    const item = weeks.get(key) || { captures: 0, evaluations: 0 };
  ...
  const max = Math.max(1, ...data.map((x) => x[1].captures));
  const x = (i) => data.length === 1 ? W/2 : P + i * ((W - 2*P)/(data.length-1));
  const y = (v) => H - P - (Number(v || 0)/max) * (H - 2*P);
  const points = data.map((item,i) => `${x(i).toFixed(1)},${y(item[1].captures).toFixed(1)}`).join(' ');
  return `<article class="sup-card sup-chart sup-line-card"><div class="sup-chart-head"><h3>Evolución semanal</h3><span>Capturas · últimas 12 semanas con datos</span></div><svg class="sup-line" viewBox="0 0 ${W} ${H}" role="img" aria-label="Capturas por semana"><line x1="${P}" y1="${H-P}" x2="${W-P}" y2="${H-P}" class="axis"/><polyline points="${points}" class="trend"/>${data.map((item,i) => `<circle cx="${x(i)}" cy="${y(item[1].captures)}" r="5" class="dotpoint"><title>${item[0]}: ${item[1].captures} capturas</title></circle><text x="${x(i)}" y="${H-10}" text-anchor="middle">${esc(item[0].replace(/^\d{4}-/,''))}</text>`).join('')}</svg></article>`;
}

function chartsMarkup(rows) {
  return `<section class="sup-chart-grid">
    ${weeklyChart(rows)}
    ${bars('Lotes con más capturas','Top 8',rankData(rows,(r)=>r.lote,(r)=>r.capturas))}
    ${bars('Capturas por fundo','Comparativo operativo',rankData(rows,(r)=>r.fundo,(r)=>r.capturas))}
    ${bars('Actividad por evaluador','Número de evaluaciones',rankData(rows,(r)=>r.evaluador || r.evaluadorId,()=>1))}
  </section>`;
}
...
function mapProjection(features) {
  const points = [];
  features.forEach((f) => geometryParts(f.geometry).forEach((poly) => poly.forEach((ring) => ring.forEach((p) => points.push(p)))));
  if (!points.length) return null;
  const xs = points.map((p) => p[0]), ys = points.map((p) => p[1]);
  const minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys);
  const W = 1040, H = 620, P = 24;
  const dx = Math.max(0.000001, maxX-minX), dy = Math.max(0.000001, maxY-minY);
  const scale = Math.min((W-2*P)/dx, (H-2*P)/dy);
...
function colorFor(item, maxMetric) {
  ...
  const light = 78 - t*35;
  return `hsl(148 46% ${light.toFixed(1)}%)`;
}
...
function mapMarkup(rows) {
  ...
  return `<section class="sup-card sup-map-section">
    ...
    <div class="sup-map-layout"><div class="sup-map-canvas">${projection ? `<svg id="supMapSvg" viewBox="0 0 ${projection.W} ${projection.H}" role="img" aria-label="Mapa de lotes fitosanitarios">${features.map((f) => {
      const lot = f.properties?.LOTE || '';
      const item = byLot.get(lot);
    ...
    }).join('')}</svg>` : '<div class="sup-empty">No hay lotes para los filtros seleccionados.</div>'}<div class="sup-map-legend"><span><i class="none"></i>Sin evaluación</span><span><i class="low"></i>Menor intensidad relativa</span><span><i class="high"></i>Mayor intensidad relativa</span></div></div>${lotDetail(selectedFeature,selectedItem)}</div>
  </section>`;
}
...
function bindDashboard(page) {
  const setFilter = (key, value) => { state.filters[key] = value; renderDashboard(page); };
  page.querySelector('#supType')?.addEventListener('change', (e) => setFilter('type', e.target.value));
  page.querySelector('#supFrom')?.addEventListener('change', (e) => setFilter('from', e.target.value));
  page.querySelector('#supTo')?.addEventListener('change', (e) => setFilter('to', e.target.value));
  page.querySelector('#supCampo')?.addEventListener('change', (e) => { state.filters.campo = e.target.value; state.filters.fundo=''; state.filters.modulo=''; state.selectedLot=''; renderDashboard(page); });
  page.querySelector('#supFundo')?.addEventListener('change', (e) => { state.filters.fundo = e.target.value; state.filters.modulo=''; state.selectedLot=''; renderDashboard(page); });
  page.querySelector('#supModulo')?.addEventListener('change', (e) => { state.filters.modulo = e.target.value; state.selectedLot=''; renderDashboard(page); });
  page.querySelector('#supEvaluator')?.addEventListener('change', (e) => setFilter('evaluator', e.target.value));
  page.querySelector('#supClear')?.addEventListener('click', () => { state.filters = { type:'all',from:'',to:'',campo:'',fundo:'',modulo:'',evaluator:'' }; state.selectedLot=''; renderDashboard(page); });
  page.querySelector('#supRefresh')?.addEventListener('click', async () => {
    const btn = page.querySelector('#supRefresh');
    if (btn) { btn.disabled = true; btn.textContent = 'Actualizando…'; }
    try { await loadData(true); renderDashboard(page); }
    catch (error) { showError(page,error); }
  });
  page.querySelectorAll('[data-sup-lot]').forEach((path) => path.addEventListener('click', () => { state.selectedLot = path.dataset.supLot || ''; renderDashboard(page); }));
}
...
function detectPage() {
  ...
  if (!page) return;
  const centralSummary = /Resumen central/i.test(page.textContent || '');
  if (centralSummary && !page.dataset.supervision) enhanceSummary(page);
}

const observer = new MutationObserver(() => queueMicrotask(detectPage));
